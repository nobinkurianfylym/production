-- FPMS — D1 schema (SQLite dialect, targets Cloudflare D1)
-- ════════════════════════════════════════════════════════════════════════
-- This replaces the single-JSON-blob storage the prototype used. Every
-- table carries production_id, and every query in the API layer filters
-- on it — cross-production data leakage is a query-layer bug here, not a
-- structural impossibility, which is why functions/api/lib/db.js wraps
-- every read/write to enforce it centrally rather than trusting each
-- endpoint to remember the WHERE clause. See NFR-DATA-002 in the SRS.
--
-- Run with:
...
CREATE TABLE users (
  ...
  email         TEXT NOT NULL,
  name          TEXT NOT NULL,
  password_hash TEXT NOT NULL,   -- PBKDF2-SHA256, see functions/api/lib/auth.js
...
CREATE TABLE sessions (
  token       TEXT PRIMARY KEY,   -- random 256-bit token, not a JWT — revocable by row delete
...
-- Role is per person per production — the same login can be 1st AD on one
-- film and 2nd AD on another (FR-ROLE-002).
...
CREATE TABLE locations (
  ...
  production_id TEXT NOT NULL REFERENCES productions(id) ON DELETE CASCADE,
  name          TEXT NOT NULL,
  sets          TEXT NOT NULL DEFAULT '[]',   -- JSON array
...
CREATE TABLE people (
  ...
  phone         TEXT DEFAULT '',
  email         TEXT DEFAULT '',
  -- PII: bank/ID fields are deliberately absent from D1. If added later,
  -- they must go in a table gated by its own permission check, not here
  -- (FR-CC-005 — masked by default, access logged).
  rate          REAL NOT NULL DEFAULT 0,
...
CREATE TABLE day_strips (
  ...
  UNIQUE (scene_id)   -- a scene can only be on one day's board at a time
);
...
CREATE TABLE dprs (
  day_id          TEXT PRIMARY KEY REFERENCES shooting_days(id) ON DELETE CASCADE,
  planned_eighths INTEGER NOT NULL DEFAULT 0,   -- frozen at approval — never re-derived
...
CREATE TABLE budget_lines (
  ...
  rate        REAL NOT NULL DEFAULT 0,
  fringe      REAL NOT NULL DEFAULT 0   -- fraction, e.g. 0.13 for 13%
);
...
-- ── Audit ──────────────────────────────────────────────────────────────
-- Append-only by convention: the API layer never issues UPDATE or DELETE
-- against this table (NFR-070). A void is a new row, not an erasure.
